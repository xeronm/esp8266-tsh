    ...
    while (entry_ptr < max_ptr) {
        ...
        if (entry->next_entry == IH_FREE_SLOT) {
            ...
            ih_entry_ptr_t *entry_ptr2 =
                d_pointer_add (ih_entry_ptr_t, hdr,
                               sizeof (ih_header8_t) + entry->next_entry * sizeof (ih_entry_ptr_t));
 ...
 */
ih_errcode_t    ICACHE_FLASH_ATTR
ih_init8 (char *buf, size_t length, uint8 bucket_size, uint8 key_length, uint8 value_length, ih_hndlr_t * hndlr)
{
...
ih_hash8_add (ih_hndlr_t hndlr, const char *entrykey, ih_size_t len, char **value, ih_size_t valuelen)
{
    ...
    entry_len += (hdr->key_length > 1) ? hdr->key_length : ((hdr->key_length) ? sizeof (size_t) : 1) + len;
