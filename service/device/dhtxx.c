...
dht_query (dht_t * value)
{
    ...
    d_log_dbprintf (DHT_SERVICE_NAME, (char *) &sample_buf, sizeof (dhtxx_resp_sample_t),
                    "sample buffer [count:%u,len10us:%u]", i, _ts);
    ...
    for (i = 0; i < DHTxx_SAMPLE_SIZE * 8; i += 1) {
        ...
        if (flevel != flevel_prev) {
            ...
            if (lidx > 2) {
                ...
                //         Data bit                                              
                if ((lidx % 2 == 1)
                    && ((!d_sample_check_len (flevel_dur, DHTxx_SLAVE_LOW_DATA_PREFIX_10US, _ts)) || flevel_prev)) {
    ...
    if (sdata->stat_last_result == DHT_RESULT_SUCCESS) {
        ...
        if (!sdata->stat_ema_initcnt) {
            if ((sdata->conf.thresh_high_signal) &&
                (((prev_ema.hmdt < sdata->conf.thresh_high.hmdt)
                  && (sdata->stat_ema_value.hmdt >= sdata->conf.thresh_high.hmdt))
                 || ((prev_ema.temp < sdata->conf.thresh_high.temp)
                     && (sdata->stat_ema_value.temp >= sdata->conf.thresh_high.temp))
                )) {
                svcctl_service_message (0, 0, NULL, sdata->conf.thresh_high_signal, NULL, NULL);
            }

            if ((sdata->conf.thresh_low_signal) &&
                (((prev_ema.hmdt > sdata->conf.thresh_low.hmdt)
                  && (sdata->stat_ema_value.hmdt <= sdata->conf.thresh_low.hmdt))
                 || ((prev_ema.temp > sdata->conf.thresh_low.temp)
                     && (sdata->stat_ema_value.temp <= sdata->conf.thresh_low.temp))
                )) {
 ...
 */
LOCAL void      ICACHE_FLASH_ATTR
fn_dht_get (sh_eval_ctx_t * evctx, sh_bc_arg_t * ret_arg, const arg_count_t arg_count, sh_bc_arg_type_t arg_type[],
            sh_bc_arg_t * bc_args[])
{
    if (arg_count != 3) {
...
dht_on_start (const svcs_resource_t * svcres, dtlv_ctx_t * conf)
{
    ...
    d_svcs_check_imdb_error (imdb_clsobj_insert
                             (svcres->hmdb, svcres->hdata, d_pointer_as (void *, &sdata), sizeof (dht_data_t))
        );
...
dht_on_msg_info (dtlv_ctx_t * msg_out)
{
    ...
    d_svcs_check_imdb_error (dtlv_avp_encode_uint8 (msg_out, COMMON_AVP_PEREPHERIAL_GPIO_ID, sdata->conf.gpio_id) ||
                             ...
                             (sdata->conf.
                              thresh_high_signal ? (dtlv_avp_encode_grouping (msg_out, 0, DHT_THRESHOLD_HIGH, &gavp)
                                                    || dtlv_avp_encode_uint16 (msg_out, DHT_HUMIDITY,
                                                                               sdata->conf.thresh_high.hmdt)
                                                    || dtlv_avp_encode_uint16 (msg_out, DHT_TEMPERATURE,
                                                                               sdata->conf.thresh_high.temp)
                                                    || dtlv_avp_encode_uint8 (msg_out, COMMON_AVP_MULTICAST_SIGNAL,
                                                                              sdata->conf.thresh_high_signal)
                                                    || dtlv_avp_encode_group_done (msg_out, gavp)) : 0)
                             || (sdata->conf.
                                 thresh_low_signal ? (dtlv_avp_encode_grouping (msg_out, 0, DHT_THRESHOLD_HIGH, &gavp)
                                                      || dtlv_avp_encode_uint16 (msg_out, DHT_HUMIDITY,
                                                                                 sdata->conf.thresh_low.hmdt)
                                                      || dtlv_avp_encode_uint16 (msg_out, DHT_TEMPERATURE,
                                                                                 sdata->conf.thresh_low.temp)
                                                      || dtlv_avp_encode_uint8 (msg_out, COMMON_AVP_MULTICAST_SIGNAL,
                                                                                sdata->conf.thresh_low_signal)
                                                      || dtlv_avp_encode_group_done (msg_out, gavp)) : 0)
        );
...
dht_on_cfgupd_thresh (dtlv_ctx_t * ctx, dht_t * thresh, uint8 * thresh_signal)
{
    ...
    if ((*thresh_signal)
        && ((*thresh_signal < SVCS_MSGTYPE_MULTICAST_MIN) || (*thresh_signal > SVCS_MSGTYPE_MULTICAST_MAX))) {
